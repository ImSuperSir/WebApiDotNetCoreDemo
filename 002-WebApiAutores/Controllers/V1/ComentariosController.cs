...
namespace _002_WebApiAutores.Controllers.V1
{
    ...
    public class ComentariosController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<List<ComentarioDTO>>> Get(int libroId, [FromQuery] PaginacionDTO paginacionDTO )  /*Es imprtante que el nombre del parametro, 
                                                                                ...
                                                                                * la url de controlador, en este caso particula de la dependencia de recursos*/
        {
            ...
            var lqueryable = context.Comentarios.Where(x => x.LibroId == libroId).AsQueryable();
            await HttpContext.InsertaParametrosPaginacionEnCabecera(lqueryable);

            var lComentarios = await lqueryable.OrderBy(x => x.Id).Paginar(paginacionDTO).ToListAsync();
